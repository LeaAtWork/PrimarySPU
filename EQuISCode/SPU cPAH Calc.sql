USE [SEA_PU]
GO
/****** Object:  StoredProcedure [dbo].[SPU_cPAH_Calc]    Script Date: 03/06/2013 12:20:53 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dbo].[SPU_cPAH_Calc]
	(
		@facility_id int
		,@ND_mult float 
		,@SDG varchar (2000) 
...
BEGIN
	...
	,TEQ varchar(200),  cnt varchar (2000),SDG varchar (200), detect_flag varchar (20),quals varchar(20))

	INSERT into @result
	SELECT  alp.action_level_code, s.sys_loc_code,  s.sample_date
		,sys_sample_code
		,t.test_id
		,s.facility_id
		, s.sample_type_code
		,s.matrix_code, r.result_unit 
		, r.reportable_result
		,sum (case when r.detect_flag = 'Y' then cast(result_text as float) * cast(alp.action_level  as float) 
			else (cast (@ND_mult as float) * cast(coalesce(r.reporting_detection_limit,result_text) as float)) * cast(alp.action_level as float) end) as 'TEQ'
...
select 
	...
	,case when r.detect_flag='N' then 'U' else r.quals end as 'lab_qualifiers'
